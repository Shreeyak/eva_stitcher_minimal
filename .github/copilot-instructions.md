...
- **`MethodChannel "com.example.eva/control"`** — camera commands: `startCamera`, `stopCamera`, `capturePhoto`, `captureStitchFrame`, `setWbLocked`, `setAfEnabled`, `setAeEnabled`, `setFocusDistance`, `setZoomRatio`, `setIso`, `setExposureTimeNs`, `setCaptureIntent`, `setCaptureFormat`, etc.
